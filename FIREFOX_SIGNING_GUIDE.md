...
Ключи Mozilla не сохраняются в проект. Скрипт спрашивает их при запуске и передает в `web-ext` через переменные окружения текущего процесса.
...
8. добавит в Firefox-манифест `update_url`;
9. проверит сборку через `web-ext lint --self-hosted`;
10. отправит сборку на Mozilla unlisted-подписание;
11. создаст `updates.json` для автообновления;
12. скопирует `.xpi`, `updates.json`, `version.json`, `INSTALL_FIREFOX.txt` и `index.html` в сетевую папку;
13. загрузит эти же файлы на HTTPS-сервер через `ssh/scp`.
