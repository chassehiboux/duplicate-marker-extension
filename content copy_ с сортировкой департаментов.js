(function() {
  ...
  const SLOWSEARCH_DEPARTMENT_SEARCH_INPUT_SELECTOR = 'input#search-department-field';
  const SLOWSEARCH_DEPARTMENT_MENU_SELECTOR = '.dropdown-menu-sub.show-depid';
  const SLOWSEARCH_DEPARTMENT_TOGGLE_STYLE_ID = 'dup-show-hidden-departments-style';
  const SLOWSEARCH_DEPARTMENT_TOGGLE_ROW_CLASS = 'dup-show-hidden-departments-toggle-row';
  const SLOWSEARCH_DEPARTMENT_TOGGLE_LABEL_CLASS = 'dup-show-hidden-departments-toggle-label';
  const SLOWSEARCH_DEPARTMENT_TOGGLE_CHECKBOX_CLASS = 'dup-show-hidden-departments-toggle-checkbox';
  const SLOWSEARCH_DEPARTMENT_WARNING_ROW_CLASS = 'dup-show-hidden-departments-warning-row';
  const SLOWSEARCH_DEPARTMENT_WARNING_TEXT_CLASS = 'dup-show-hidden-departments-warning-text';
  const SLOWSEARCH_DEPARTMENT_SHOW_HIDDEN_CLASS = 'dup-show-hidden-departments';
  const SLOWSEARCH_DEPARTMENT_HIDDEN_ATTR = 'data-dup-hidden-department';
  const SLOWSEARCH_DEPARTMENT_TOGGLE_TEXT = 'Показать скрытые департаменты';
  const SLOWSEARCH_DEPARTMENT_WARNING_PREFIX = 'Не найдены depid из массива: ';
  // Порядок в этом массиве определяет порядок отображения depid в списке.
  const SLOWSEARCH_ALLOWED_DEPIDS_ORDER = [
    '39',
    '43',
    '40',
    '61',
  ...
  const SLOWSEARCH_ALLOWED_DEPIDS = new Set(SLOWSEARCH_ALLOWED_DEPIDS_ORDER);
  const SLOWSEARCH_ALLOWED_DEPID_ORDER_INDEX = new Map(
    SLOWSEARCH_ALLOWED_DEPIDS_ORDER.map((depid, index) => [String(depid), index])
  );
  ...
  let slowsearchShowHiddenDepartments = false;
  ...
  function ensureScreenshotHideStyle() {
    ...
    style.textContent = `
      ...
      html.${SCREENSHOT_MODE_CLASS} .${SLOWSEARCH_DEPARTMENT_TOGGLE_ROW_CLASS},
      html.${SCREENSHOT_MODE_CLASS} .${SLOWSEARCH_DEPARTMENT_WARNING_ROW_CLASS},
  ...
  function ensureSlowsearchDepartmentToggleStyle() {
    if (document.getElementById(SLOWSEARCH_DEPARTMENT_TOGGLE_STYLE_ID)) return;

    const style = document.createElement('style');
    style.id = SLOWSEARCH_DEPARTMENT_TOGGLE_STYLE_ID;
    style.textContent = `
      .${SLOWSEARCH_DEPARTMENT_TOGGLE_ROW_CLASS} {
        padding: 6px 12px 4px;
      }

      .${SLOWSEARCH_DEPARTMENT_TOGGLE_LABEL_CLASS} {
        display: inline-flex;
        align-items: center;
        gap: 8px;
        margin: 0;
      ...
      .${SLOWSEARCH_DEPARTMENT_TOGGLE_CHECKBOX_CLASS} {
        margin: 0;
      }

      .${SLOWSEARCH_DEPARTMENT_WARNING_ROW_CLASS} {
        padding: 0 12px 6px;
      }

      .${SLOWSEARCH_DEPARTMENT_WARNING_TEXT_CLASS} {
        display: block;
        margin: 0;
        font-size: 11px;
        line-height: 1.3;
      ...
      ${SLOWSEARCH_DEPARTMENT_MENU_SELECTOR}:not(.${SLOWSEARCH_DEPARTMENT_SHOW_HIDDEN_CLASS}) li[${SLOWSEARCH_DEPARTMENT_HIDDEN_ATTR}="1"] {
        display: none !important;
      }
    `;
    (document.head || document.documentElement).appendChild(style);
  ...
  function getSlowsearchDepartmentDropdownContents() {
    return Array.from(document.querySelectorAll('.dropdown-content')).filter((content) => {
      if (!(content instanceof HTMLElement)) return false;
      return !!content.querySelector(SLOWSEARCH_DEPARTMENT_SEARCH_INPUT_SELECTOR)
        && !!content.querySelector(SLOWSEARCH_DEPARTMENT_MENU_SELECTOR);
    });
  }

  function ensureSlowsearchDepartmentToggle(dropdownContent) {
    ...
    const departmentsMenu = dropdownContent.querySelector(SLOWSEARCH_DEPARTMENT_MENU_SELECTOR);
    if (!(headerItem instanceof HTMLLIElement) || !(departmentsMenu instanceof HTMLElement)) return null;

    let toggleRow = dropdownContent.querySelector(`.${SLOWSEARCH_DEPARTMENT_TOGGLE_ROW_CLASS}`);
    if (!(toggleRow instanceof HTMLLIElement)) {
      toggleRow = document.createElement('li');
      toggleRow.className = SLOWSEARCH_DEPARTMENT_TOGGLE_ROW_CLASS;

      const toggleLabel = document.createElement('label');
      toggleLabel.className = SLOWSEARCH_DEPARTMENT_TOGGLE_LABEL_CLASS;

      const toggleCheckbox = document.createElement('input');
      toggleCheckbox.type = 'checkbox';
      toggleCheckbox.className = SLOWSEARCH_DEPARTMENT_TOGGLE_CHECKBOX_CLASS;
      toggleCheckbox.checked = slowsearchShowHiddenDepartments;
      toggleCheckbox.addEventListener('change', () => {
        slowsearchShowHiddenDepartments = toggleCheckbox.checked;
        syncSlowsearchDepartmentVisibility();
      });

      const toggleText = document.createElement('span');
      toggleText.textContent = SLOWSEARCH_DEPARTMENT_TOGGLE_TEXT;
    ...
    const toggleCheckbox = toggleRow.querySelector(`.${SLOWSEARCH_DEPARTMENT_TOGGLE_CHECKBOX_CLASS}`);
    if (toggleCheckbox instanceof HTMLInputElement && toggleCheckbox.checked !== slowsearchShowHiddenDepartments) {
      toggleCheckbox.checked = slowsearchShowHiddenDepartments;
    }
  ...
  function updateSlowsearchDepartmentMissingWarning(dropdownContent, missingDepids) {
    if (!(dropdownContent instanceof HTMLElement)) return;

    let warningRow = dropdownContent.querySelector(`.${SLOWSEARCH_DEPARTMENT_WARNING_ROW_CLASS}`);
    const safeMissingDepids = Array.isArray(missingDepids)
      ? missingDepids.filter((depid) => String(depid || '').trim().length > 0)
      : [];

    if (safeMissingDepids.length === 0) {
      if (warningRow instanceof HTMLElement) warningRow.remove();
      return;
    }

    if (!(warningRow instanceof HTMLLIElement)) {
      warningRow = document.createElement('li');
      warningRow.className = SLOWSEARCH_DEPARTMENT_WARNING_ROW_CLASS;
    }

    let warningText = warningRow.querySelector(`.${SLOWSEARCH_DEPARTMENT_WARNING_TEXT_CLASS}`);
    if (!(warningText instanceof HTMLSpanElement)) {
      warningRow.textContent = '';
      warningText = document.createElement('span');
      warningText.className = SLOWSEARCH_DEPARTMENT_WARNING_TEXT_CLASS;
      warningRow.appendChild(warningText);
    }

    warningText.textContent = `${SLOWSEARCH_DEPARTMENT_WARNING_PREFIX}${safeMissingDepids.join(', ')}`;

    const toggleRow = dropdownContent.querySelector(`.${SLOWSEARCH_DEPARTMENT_TOGGLE_ROW_CLASS}`);
    if (
      toggleRow instanceof HTMLElement &&
      (warningRow.parentElement !== dropdownContent || warningRow.previousElementSibling !== toggleRow)
    ) {
      toggleRow.insertAdjacentElement('afterend', warningRow);
      return;
    }

    if (warningRow.parentElement !== dropdownContent) {
      dropdownContent.appendChild(warningRow);
    }
  }

  function parseSlowsearchDepidSortValue(value) {
    const depid = String(value || '').trim();
    if (!depid) return Number.NaN;
    const parsed = Number.parseInt(depid, 10);
    return Number.isFinite(parsed) ? parsed : Number.NaN;
  }

  function compareSlowsearchDepartmentItems(leftItem, rightItem) {
    ...
    const leftDepidRaw = String(leftLink && leftLink.getAttribute('data-depid') || '').trim();
    const rightDepidRaw = String(rightLink && rightLink.getAttribute('data-depid') || '').trim();
    const leftOrderIndex = SLOWSEARCH_ALLOWED_DEPID_ORDER_INDEX.get(leftDepidRaw);
    const rightOrderIndex = SLOWSEARCH_ALLOWED_DEPID_ORDER_INDEX.get(rightDepidRaw);
    ...
    const leftDepid = parseSlowsearchDepidSortValue(leftDepidRaw);
    const rightDepid = parseSlowsearchDepidSortValue(rightDepidRaw);
    const hasLeftDepid = Number.isFinite(leftDepid);
    const hasRightDepid = Number.isFinite(rightDepid);
    if (hasLeftDepid && hasRightDepid && leftDepid !== rightDepid) return leftDepid - rightDepid;
    if (hasLeftDepid && !hasRightDepid) return -1;
    if (!hasLeftDepid && hasRightDepid) return 1;

    const leftText = String(leftLink ? leftLink.textContent : leftItem.textContent || '')
      .replace(/\s+/g, ' ')
      .trim();
    const rightText = String(rightLink ? rightLink.textContent : rightItem.textContent || '')
      .replace(/\s+/g, ' ')
      .trim();
    return leftText.localeCompare(rightText, 'ru', { sensitivity: 'base', numeric: true });
  }

  function sortSlowsearchDepartmentItems(departmentsMenu) {
    if (!(departmentsMenu instanceof HTMLElement)) return;

    const departmentItems = Array.from(departmentsMenu.children)
      .filter((node) => node instanceof HTMLLIElement);
    if (departmentItems.length < 2) return;

    const sortedItems = departmentItems.slice().sort(compareSlowsearchDepartmentItems);
    sortedItems.forEach((item) => {
      departmentsMenu.appendChild(item);
    });
  }

  function updateSlowsearchDepartmentVisibilityForDropdown(dropdownContent) {
    if (!(dropdownContent instanceof HTMLElement)) return;

    const departmentsMenu = dropdownContent.querySelector(SLOWSEARCH_DEPARTMENT_MENU_SELECTOR);
    if (!(departmentsMenu instanceof HTMLElement)) return;

    const departmentLinks = Array.from(departmentsMenu.querySelectorAll(SLOWSEARCH_CITIES_LINK_SELECTOR))
      .filter((link) => link instanceof HTMLAnchorElement);
    const presentDepids = new Set();

    departmentLinks.forEach((link) => {
      const hostItem = link.closest('li') || link;
      if (!(hostItem instanceof HTMLElement)) return;

      const depid = String(link.getAttribute('data-depid') || '').trim();
      if (depid) presentDepids.add(depid);
      if (SLOWSEARCH_ALLOWED_DEPIDS.has(depid)) {
        hostItem.removeAttribute(SLOWSEARCH_DEPARTMENT_HIDDEN_ATTR);
      } else {
        hostItem.setAttribute(SLOWSEARCH_DEPARTMENT_HIDDEN_ATTR, '1');
      }
    });

    const missingDepids = SLOWSEARCH_ALLOWED_DEPIDS_ORDER.filter((depid) => !presentDepids.has(depid));
    updateSlowsearchDepartmentMissingWarning(dropdownContent, missingDepids);
    sortSlowsearchDepartmentItems(departmentsMenu);
    departmentsMenu.classList.toggle(SLOWSEARCH_DEPARTMENT_SHOW_HIDDEN_CLASS, slowsearchShowHiddenDepartments);
  }

  function syncSlowsearchDepartmentVisibility() {
    ensureSlowsearchDepartmentToggleStyle();
    const dropdownContents = getSlowsearchDepartmentDropdownContents();
    dropdownContents.forEach((dropdownContent) => {
      ensureSlowsearchDepartmentToggle(dropdownContent);
      updateSlowsearchDepartmentVisibilityForDropdown(dropdownContent);
    });
  }

  const debouncedSyncSlowsearchDepartmentVisibility = debounce(syncSlowsearchDepartmentVisibility, 120);

  function initSlowsearchDepartmentVisibility() {
    syncSlowsearchDepartmentVisibility();

    const observer = new MutationObserver(() => {
      debouncedSyncSlowsearchDepartmentVisibility();
    });
    observeWithRetry(
      observer,
      () => document.body || document.documentElement,
  ...
  initSlowsearchDepartmentVisibility();
