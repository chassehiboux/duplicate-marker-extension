document.addEventListener('DOMContentLoaded', () => {
  ...
  // =============================================================
  // === ВКЛАДКА 2: УПРАВЛЕНИЕ СТОЛБЦАМИ (новая логика) ========
  // =============================================================
  const columnsListDiv = document.getElementById('columns-list');
  const COL_STORAGE_KEY = 'hidden_columns'; // Используем local storage

  let currentColumns = [];
  let hiddenColumns = []; // Глобальные настройки
  ...
  async function handleCheckboxChange(event) {
    ...
    // Сохраняем в local storage и отправляем сообщение
    await chrome.storage.local.set({ [COL_STORAGE_KEY]: hiddenColumns });
    // Явное сообщение для немедленного обновления
    chrome.tabs.sendMessage(targetTabForColumns.id, {
      action: 'updateColumnVisibility',
      hiddenColumns: hiddenColumns
    }).catch(err => console.error("Failed to send visibility update:", err));
  }
  ...
  async function initColumnManager() {
    ...
    try {
      currentColumns = await chrome.tabs.sendMessage(targetTabForColumns.id, { action: 'getColumns' });
      renderColumns();
    } catch (error) {
      console.error("Error getting columns:", error);
      columnsListDiv.innerHTML = `<p>Не удалось получить столбцы с вкладки. Убедитесь, что расширение имеет доступ к странице и обновите ее. Ошибка: ${error.message}</p>`;
    }
  }

  // =============================================================
  // === ВКЛАДКА 1: СЧЕТЧИКИ И НАСТРОЙКИ (старая логика) ========
  // =============================================================

  const todayCountEl = document.getElementById('today-count');
  if(todayCountEl) { // Запускаем логику счетчиков только если элементы существуют
    ...
    // --- Остальная логика для счетчиков и настроек ---
    ...
    function updateEditingCounterUI() {
      chrome.storage.local.get([EDITING_STATS_KEY, PROCESSED_EDITS_KEY, TAGS_KEY], (result) => {
        ...
        Object.keys(todayProcessed).forEach(path => {
            ...
            if (typeof item === 'object' && item !== null && 'base_count' in item) { todayTotalFromDetails += item.base_count; } 
            else { todayTotalFromDetails += typeof item === 'number' ? item : (Array.isArray(item) ? item.length : 0); }
        });
    ...
    function renderDetails(processedData, container, tags, dateISO) {
        container.innerHTML = '';
        if (!processedData || Object.keys(processedData).length === 0) { container.innerHTML = '<div class="history-item" style="justify-content: center; opacity: 0.7;">Нет данных</div>'; return; }
        const paths = Object.keys(processedData).sort();
        let hasVisibleItems = false;
        paths.forEach(path => {
            const item = processedData[path];
            const count = (typeof item === 'object' && item !== null && 'base_count' in item) ? item.base_count : (typeof item === 'number' ? item : (Array.isArray(item) ? item.length : 0));
            if (count > 0) {
        ...
        if (!hasVisibleItems) { container.innerHTML = '<div class="history-item" style="justify-content: center; opacity: 0.7;">Нет данных</div>'; }
    }

    function renderEditingHistory(history, processed, tags, container) {
        container.innerHTML = '';
        const dates = Object.keys(history).sort((a, b) => { const [d1, m1, y1] = a.split('.'); const [d2, m2, y2] = b.split('.'); return new Date(`${y2}-${m2}-${d2}`) - new Date(`${y1}-${m1}-${d1}`); });
        ...
        dates.forEach(date => {
          ...
          if (history[date] > 0) {
            ...
            summary.addEventListener('click', () => { details.style.display = details.style.display === 'none' ? 'block' : 'none'; });
            historyGroup.appendChild(summary);
            historyGroup.appendChild(details);
            container.appendChild(historyGroup);
          }
        });
        if (!hasHistory) { container.innerHTML = '<div class="history-item" style="justify-content: center; opacity: 0.7;">История пуста</div>'; }
    }
    ...
    function renderPendingActions() {
      chrome.storage.local.get([PENDING_ACTIONS_KEY, TAGS_KEY], (result) => {
          ...
          if (pending.length === 0) { pendingActionsTitle.style.display = 'none'; return; }
    ...
    function convertDateToISO(dateString) { const [day, month, year] = dateString.split('.'); return `${year}-${month}-${day}`; }
    ...
    function modifyEditingCounter(amount) {
        chrome.storage.local.get([EDITING_STATS_KEY, PROCESSED_EDITS_KEY], (result) => {
            ...
            if (typeof processed[todayISO][MANUAL_KEY] !== 'object') { processed[todayISO][MANUAL_KEY] = { base_count: 0, unique_edocids: [] }; }
            processed[todayISO][MANUAL_KEY].base_count += amount;
            if (processed[todayISO][MANUAL_KEY].base_count < 0) processed[todayISO][MANUAL_KEY].base_count = 0;
    ...
    function recalculateTotals(processed, history, dateISO) {
        ...
        if (processed[dateISO]) {
            Object.keys(processed[dateISO]).forEach(path => {
                ...
                if (typeof item === 'object' && item !== null && 'base_count' in item) { dayTotal += item.base_count; } 
                else { dayTotal += typeof item === 'number' ? item : (Array.isArray(item) ? item.length : 0); }
            });
        }
    ...
    function handleEditingActions(e) {
        ...
        if (target.classList.contains('tag-btn')) {
            chrome.storage.local.get(TAGS_KEY, (result) => {
                ...
                if (newTag !== null) { tags[path] = newTag.trim(); chrome.storage.local.set({ [TAGS_KEY]: tags }); }
            });
            return;
        }
        ...
        if (target.classList.contains('edit-btn')) {
            ...
            const saveChanges = () => {
                ...
                } else { container.replaceChild(countSpan, input); container.replaceChild(editBtn, saveBtn); }
            };
    ...
    pendingActionsContainer.addEventListener('click', (e) => {
      ...
      if (target.classList.contains('approve-btn')) {
          ...
          if (newTag !== null) { chrome.runtime.sendMessage({ action: 'approve_action', data: { path, edocid, tag: newTag.trim() } }); }
      }
      if (target.classList.contains('block-btn')) { chrome.runtime.sendMessage({ action: 'block_action', data: { path, edocid } }); }
    });

    btnEditingPlus.addEventListener('click', () => modifyEditingCounter(1));
    btnEditingMinus.addEventListener('click', (e) => {
        if (e.shiftKey) { modifyEditingCounter(-Infinity); } 
        else { modifyEditingCounter(-1); }
    });
    editingHistoryBtn.addEventListener('click', () => { editingDetailsContainer.style.display = 'none'; editingHistoryContainer.style.display = (editingHistoryContainer.style.display === 'block') ? 'none' : 'block'; });
    editingDetailsBtn.addEventListener('click', () => { editingHistoryContainer.style.display = 'none'; editingDetailsContainer.style.display = (editingDetailsContainer.style.display === 'block') ? 'none' : 'block'; });
    ...
    const allSettings = [...inputsSearch, ...inputsStrict, 'setting_copy_mode', 'setting_highlight_mode', 'setting_notify_execution', 'setting_notify_editing'];

    chrome.storage.local.get(allSettings, (items) => {
      if (items['setting_copy_mode'] === undefined) { items['setting_copy_mode'] = true; chrome.storage.local.set({ 'setting_copy_mode': true }); }
      if (items['setting_notify_execution'] === undefined) { items['setting_notify_execution'] = true; chrome.storage.local.set({ 'setting_notify_execution': true }); }
      if (items['setting_notify_editing'] === undefined) { items['setting_notify_editing'] = true; chrome.storage.local.set({ 'setting_notify_editing': true }); }
      inputsSearch.forEach(id => { if (items[id] === undefined) { items[id] = true; chrome.storage.local.set({ [id]: true }); } });
      ...
      inputsSearch.forEach(id => { const el = document.getElementById(id); if (el) el.checked = items[id]; });
      inputsStrict.forEach(id => { const el = document.getElementById(id); if (el) el.checked = items[id] || false; });
    });

    function createSettingChangeListener(settingId) {
      const element = document.getElementById(settingId);
      if (element) { element.addEventListener('change', (e) => { const value = e.target.type === 'checkbox' ? e.target.checked : e.target.value; chrome.storage.local.set({ [settingId]: value }); }); }
    }

    allSettings.forEach(createSettingChangeListener);
    ...
    function updateLogsUI(logs) {
      ...
      if (logs && logs.length > 0) { logs.forEach(logMsg => { const logItem = document.createElement('div'); logItem.className = 'log-entry'; logItem.textContent = logMsg; logContainer.appendChild(logItem); });
      } else { logContainer.textContent = 'Журнал пуст.'; }
    }

    function loadLogs() { chrome.storage.local.get(LOG_KEY, (data) => { updateLogsUI(data[LOG_KEY]); }); }

    if (btnClearLog) { btnClearLog.addEventListener('click', () => { chrome.storage.local.remove(LOG_KEY, () => { loadLogs(); }); }); }
    
    // --- ПЕРВИЧНАЯ ЗАГРУЗКА И ГЛОБАЛЬНЫЕ СЛУШАТЕЛИ ---
    updateCounterUI();
    ...
    chrome.storage.onChanged.addListener((changes) => {
      ...
      if (changes[EDITING_STATS_KEY] || changes[PROCESSED_EDITS_KEY] || changes[TAGS_KEY] || changes[PENDING_ACTIONS_KEY]) { updateEditingCounterUI(); renderPendingActions(); }
      if (changes[LOG_KEY]) updateLogsUI(changes[LOG_KEY].newValue);
    });

    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      if (message.action === "updateEditingCounter" || message.action === "pendingActionsUpdated") { updateEditingCounterUI(); renderPendingActions(); }
    });
  }
});